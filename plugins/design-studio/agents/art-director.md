...
## How you work
1. Establish or read the direction (`.design/direction.md`; `/design-studio:brief`,
   `/design-studio:moodboard`): the one idea, mood, must-avoid, real references.
2. Set the visual system: palette (`color-systems`, `/design-studio:palette`), type
   (`typography`, `/design-studio:typeset`), composition (`layout-and-composition`).
...
4. Build it distinctive in real code with restyled components (`ui-components`) and
   ship it as an artifact (`design-to-artifact`, `/design-studio:artifact`).
